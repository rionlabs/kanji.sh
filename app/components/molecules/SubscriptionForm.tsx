...
import { SubscribedAnimation } from 'app/components/atoms/AnimatedImage';
import type { ActionData as SubscriptionFormResponse } from 'app/routes/api/subscribe';
            ...
            <submission.Form
                ...
                <fieldset className='flex flex-col items-stretch gap-4' disabled={isSubmitting}>
                    <input
                        ...
                        aria-label='Name'
                    />
                    <em aria-hidden={isSubmitting && !!subResponse?.errors?.name}
                        className='text-red-800 transition-all text-sm'>{subResponse?.errors?.name}</em>
                    <input
                        className='field'
                        name='email'
                        type='email'
                        ...
                        aria-label='Email'
                    />
                    <em aria-hidden={isSubmitting && !!subResponse?.errors?.email}
                        className='text-red-800 transition-all text-sm'>{subResponse?.errors?.email}</em>
                    <button
                        type='submit'
                        className='button px-6 mt-4'>
