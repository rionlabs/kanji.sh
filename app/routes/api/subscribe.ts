...
export const action: ActionFunction = async ({ request }) => {
    ...
    if (name.trim().length === 0) {
        return json<ActionData>({ status: 'error', errors: { name: 'Your name is necessary.' } }, 401);
    }
    ...
    if (!email || email.length === 0) {
        ...
        return json<ActionData>({ status: 'error', errors: { email: 'Valid email, please.' } }, 401);
    }
...
const addContact = async ({ name, email }: Subscription): Promise<void> => {
    ...
    **/
    ...
    console.log('[End] API Call: API called successfully. Returned data: ' + JSON.stringify(response, null, 3));
};
